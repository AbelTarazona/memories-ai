class MemoryModel {
  ...
  factory MemoryModel.fromMap(Map<String, dynamic> map) {
    return MemoryModel(
      ...
      aiFeelings: (map['ai_feelings'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      aiRepresentativeMoments: (map['ai_representative_moments'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      aiImagePrompt: map['ai_image_prompt'] as String?,
      aiPeople: (map['ai_people'] as List?)?.map((e) => e.toString()).toList(),
      aiPeopleRoles: (map['ai_people_roles'] as Map?)?.map(
        (k, v) => MapEntry(k.toString(), v),
      ),
      aiPlaces: (map['ai_places'] as List?)?.map((e) => e.toString()).toList(),
      aiObjects: (map['ai_objects'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      aiActions: (map['ai_actions'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      aiTemporalContext: map['ai_temporal_context'] as String?,
      aiOverallTone: map['ai_overall_tone'] as String?,
      aiCategory: (map['ai_category'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      aiKeyTopics: (map['ai_key_topics'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      ...
      aiSensorialElements: (map['ai_sensorial_elements'] as List?)
          ...
          .toList(),
      aiLessonsLearned: (map['ai_lessons_learned'] as List?)
          ?.map((e) => e.toString())
          .toList(),
      ...
      isEnable: map['is_enable'] as bool? ?? true,
      isTruthful: map['isTruthful'] as bool? ?? false,
    );
  }
  ...
  MemoryModel copyWith({
    ...
    return MemoryModel(
      ...
      aiRepresentativeMoments:
          aiRepresentativeMoments ?? this.aiRepresentativeMoments,
      ...
      normalizedDate: normalizedDate ?? this.normalizedDate,
      isTruthful: isTruthful ?? this.isTruthful,
      isEnable: isEnable ?? this.isEnable,
    );
  }

  /// Converts feelings to emojis
  List<String> get feelingEmojis {
    if (aiFeelings == null || aiFeelings!.isEmpty) return ['❓'];
    return aiFeelings!.map((feeling) {
      switch (feeling) {
        case 'Alegría':
          return '😊';
        case 'Tristeza':
